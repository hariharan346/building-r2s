...
const VendorRegisterPage = () => {
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", password: "", shopName: "", shopDescription: "", location: "", experience: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    const success = await register({ ...formData, type: "vendor", serviceIds: selectedServices, isAvailable: true });
    setIsLoading(false);

    if (success) {
      toast({ title: "Welcome!", description: "Your shop has been registered." });
      ...
      <main className="py-10 px-4">
        <div className="container max-w-2xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-foreground">Register Your Shop</h1>
            <p className="text-muted-foreground mt-2">Join ServiConnect as a service provider</p>
          </div>
          ...
          <div className="bg-card rounded-2xl p-6 border border-border shadow-md">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Textarea value={formData.shopDescription} onChange={(e) => setFormData({ ...formData, shopDescription: e.target.value })} rows={3} />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
            ...
            <p className="text-center text-sm text-muted-foreground mt-6">
              Already registered? <Link to="/auth/login" className="text-primary hover:underline">Login</Link>
            </p>
          </div>
        </div>
      </main>
