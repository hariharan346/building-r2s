...
import { VendorShop, getServiceById } from "@/data/services";
...
export const VendorCard = ({ vendor, serviceId, index = 0 }: VendorCardProps) => {
  return (
    <div
      ...
      <div className="flex flex-col lg:flex-row lg:items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm mt-3">
            ...
            <span className="font-semibold text-primary">{vendor.priceRange}</span>
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2 lg:w-40">
          <Button asChild variant="default" className="w-full">
            <Link to={`/vendor/${vendor.id}${serviceId ? `?service=${serviceId}` : ""}`}>
              View Profile
            </Link>
          </Button>
          {vendor.isAvailable && (
            <Button asChild variant="hero" className="w-full">
              <Link to={`/request/${vendor.id}${serviceId ? `?service=${serviceId}` : ""}`}>
                Request Service
              </Link>
            </Button>
          )}
